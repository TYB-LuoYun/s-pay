...
public enum WxTransactionType  implements TransactionType {
    ...
    JSAPI("/v3/pay{partner}/transactions/jsapi", MethodType.POST) {
        @Override
        public void setAttribute(Map<String, Object> parameters, WxPayRequest order) {
    ...
    DIVISION("/v3/profitsharing/orders", MethodType.POST),


    /**
     * 完结分账,调用订单的完结接口
     */
    DIVISION_FINISH("/v3/profitsharing/orders/unfreeze",MethodType.POST),

    /**
     * 绑定关系
     */
    DIVISION_BIND("/v3/profitsharing/receivers/add",MethodType.POST),


    /**
     * 查询分账
     */
    DIVISION_QUERY("/v3/profitsharing/orders/{out_refund_no}", MethodType.GET),
     ...
     * 获取平台证书
     */
    Certificate("/v3/certificates",MethodType.GET );
